...
class DriveContextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        context = img + 0
        H, W = img.shape[1:3]
        h = np.random.randint(H - PATCHSIZE)
        w = np.random.randint(W - PATCHSIZE)
        context[:, h:h+PATCHSIZE, w:w+PATCHSIZE] = 0

        return {
            'gt': torch.FloatTensor(img),
            'image': torch.FloatTensor(context),
