...
class BottleneckUp(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, upsample=None, groups=1,
        ...
        self.conv1 = conv1x1(inplanes, width)
        self.bn1 = norm_layer(width)
        self.conv2 = conv3x3up(width, width, stride, groups, dilation)
        self.bn2 = norm_layer(width)
        self.conv3 = conv1x1(width, planes * self.expansion)
        self.bn3 = norm_layer(planes * self.expansion)
        self.relu = nn.LeakyReLU(inplace=True)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, inp_channels=1, out_channels=4, num_classes=1000, zero_init_residual=False,
        ...
        # Get upsampling blocks as well
        if block == BasicBlock:
            upblock = BasicBlockUp
        else:
            upblock = BottleneckUp
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, Bottleneck) or isinstance(m, BottleneckUp):
                    nn.init.constant_(m.bn3.weight, 0)
                elif isinstance(m, BasicBlock) or isinstance(m, BasicBlockUp):
...
def _resnet(arch, block, layers, inp_channels, out_channels, pretrained, progress, **kwargs):
    model = ResNet(block, layers, inp_channels, out_channels, **kwargs)
    return model


def resnet18(inp_channels, out_channels, pretrained=False, progress=True, **kwargs):
    ...
    """
    return _resnet('resnet18', BasicBlock, [2, 2, 2, 2], inp_channels, out_channels, pretrained, progress,
                   **kwargs)


def resnet34(inp_channels, out_channels, pretrained=False, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet34', BasicBlock, [3, 4, 6, 3], inp_channels, out_channels, pretrained, progress,
                   **kwargs)


def resnet50(inp_channels, out_channels, pretrained=False, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet50', Bottleneck, [3, 4, 6, 3], inp_channels, out_channels, pretrained, progress,
                   **kwargs)
