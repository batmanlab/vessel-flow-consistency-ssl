        ...
        for batch_idx, data in enumerate(self.data_loader):
            ...
            if batch_idx % self.log_step == 0:
                self.logger.debug('Train Epoch: {} {} Loss: {:.6f}'.format(
                    ...
                    loss.item()))
                self.writer.add_image('input', make_grid(0.5 + 0.5*data['image'].cpu(), nrow=8, normalize=True))
                self.writer.add_image('recon', make_grid(0.5 + 0.5*output['recon'].cpu(), nrow=8, normalize=True))
